...
interface PreviewDrawerProps {
  ...
  css: string;
  js: string;
  fullHeight?: boolean; // New prop
}

const DEFAULT_CONTENT_HEIGHT = 280;
const MAX_HEIGHT_RATIO = 0.8;

export function PreviewDrawer({ html, css, js, fullHeight = false }: PreviewDrawerProps) {
  const [contentHeight, setContentHeight] = useState(DEFAULT_CONTENT_HEIGHT);
  const [isDragging, setIsDragging] = useState(false);
  const [dynamicBg, setDynamicBg] = useState('#ffffff');
  ...
  const handleDragStart = useCallback(
    (clientY: number) => {
      if (!containerRef.current || fullHeight) return; // Prevent drag if fullHeight
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      if (!containerRef.current || !dragState.current || fullHeight) return;
  ...
  const handleEnd = useCallback(() => {
    if (containerRef.current && dragState.current && !fullHeight) {
  ...
  useEffect(() => {
    if (!isDragging || fullHeight) return;
  ...
  }, [isDragging, fullHeight, handleMouseMove, handleTouchMove, handleEnd]);
  ...
  const togglePreview = () => {
    if (fullHeight) return; // Disable toggle for fullHeight
    setContentHeight(prev => (prev > 5 ? 0 : DEFAULT_CONTENT_HEIGHT));
  };

  const isCollapsed = contentHeight <= 5;

  // If fullHeight is true, we force 100% height, otherwise use the dynamic height
  const totalHeight = fullHeight ? '100%' : HANDLE_HEIGHT + contentHeight;
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className={`${fullHeight ? 'absolute inset-0' : 'fixed left-0 right-0'} border-t border-[#1a1a1a] flex flex-col z-[100] ${
          isDragging ? '' : 'transition-[height] duration-200 ease-out'
        }`}
        style={{
          height: totalHeight,
          bottom: fullHeight ? '0' : `${FOOTER_HEIGHT}px`,
          willChange: fullHeight ? 'auto' : 'height',
        }}
      >
        {/* Handle - Only show if NOT fullHeight */}
        {!fullHeight && (
          <div
